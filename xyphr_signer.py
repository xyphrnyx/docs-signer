...
APP_NAME = "Xyphrnyx PDF Signer"
APP_VERSION = "1.0.0"
MANIFEST_NAME = "XYPHERNYX_PROVENANCE_MANIFEST"
MANIFEST_VERSION = "1.0.0"
MANIFEST_PROFILE = "APM-1"
DEFAULT_AUTHOR = "Xyphrnyx"
DEFAULT_CONTACT = "alessamiau@icloud.com"
DEFAULT_KEY_ID = "Xyphrnyx-ED25519-2026-01"
DEFAULT_ENGINE_FAMILY = "XYPHERNYX_SIGNING_ENGINES"
SIGNED_SUFFIX = "_firmado"

LOCAL_MODES = ("declared", "hash", "sign-local", "auto")


class XyphrnyxError(RuntimeError):
...
def canonical_json_bytes(value: Any) -> bytes:
    def encode(item: Any) -> str:
        ...
        if isinstance(item, float):
            raise XyphrnyxError("El manifiesto contiene un numero flotante. APM-1 exige enteros o strings.")
        ...
        if isinstance(item, dict):
            if not all(isinstance(k, str) for k in item):
                raise XyphrnyxError("Todas las claves JSON deben ser strings.")
            keys = sorted(item.keys(), key=_utf16_sort_key)
            return "{" + ",".join(f"{encode(key)}:{encode(item[key])}" for key in keys) + "}"
        raise XyphrnyxError(f"Tipo no compatible con canonicalizacion: {type(item)!r}")
...
def resolve_local_mode(args: argparse.Namespace) -> tuple[str, Ed25519PrivateKey | None]:
    ...
    if mode == "sign-local":
        if private_path is None:
            raise XyphrnyxError("--private-key es obligatorio en modo sign-local.")
    ...
    if mode == "auto":
        if private_path and private_path.exists():
            ...
            return "sign-local", load_private_key(private_path, password)
        return "hash", None
    raise XyphrnyxError(f"Modo no reconocido: {mode}")


def process_one_local(source_path: Path, args: argparse.Namespace) -> dict[str, Any]:
    if not source_path.exists() or not source_path.is_file():
        raise XyphrnyxError(f"No existe el PDF: {source_path}")
    if source_path.suffix.lower() != ".pdf":
        raise XyphrnyxError(f"El archivo no es PDF: {source_path}")
    if source_path.stem.endswith(SIGNED_SUFFIX):
        raise XyphrnyxError("El archivo ya parece ser una salida Xyphrnyx firmada.")
    ...
    if paths["pdf"].exists() and not args.overwrite:
        raise XyphrnyxError(f"Ya existe {paths['pdf']}. Usa --overwrite para reemplazarla.")
...
def save_keypair(
    ...
    for path in (private_path, public_path):
        if path.exists() and not overwrite:
            raise XyphrnyxError(f"Ya existe {path}. Usa --overwrite solo si realmente deseas reemplazarlo.")
...
def load_private_key(path: Path, password: bytes | None) -> Ed25519PrivateKey:
    if not path.exists():
        raise XyphrnyxError(f"No existe la clave privada: {path}")
    try:
        key = serialization.load_pem_private_key(path.read_bytes(), password=password)
    except (TypeError, ValueError) as exc:
        raise XyphrnyxError("No fue posible abrir la clave privada. Verifica la contrasena.") from exc
    if not isinstance(key, Ed25519PrivateKey):
        raise XyphrnyxError("La clave privada no es Ed25519.")
    return key


def load_public_key(path: Path) -> Ed25519PublicKey:
    if not path.exists():
        raise XyphrnyxError(f"No existe la clave publica: {path}")
    try:
        key = serialization.load_pem_public_key(path.read_bytes())
    except ValueError as exc:
        raise XyphrnyxError("No fue posible abrir la clave publica.") from exc
    if not isinstance(key, Ed25519PublicKey):
        raise XyphrnyxError("La clave publica no es Ed25519.")
    return key
...
def resolve_password(args: argparse.Namespace, *, confirm: bool = False) -> bytes | None:
    ...
    if env_name:
        ...
        if value is None:
            raise XyphrnyxError(f"No existe la variable de entorno {env_name}.")
        return value.encode("utf-8")

    first = getpass.getpass("Contrasena de la clave privada: ")
    if not first:
        raise XyphrnyxError("La contrasena no puede estar vacia.")
    if confirm:
        second = getpass.getpass("Repite la contrasena: ")
        if first != second:
            raise XyphrnyxError("Las contrasenas no coinciden.")
    ...
    return {
        ...
        "authorship": {
            ...
            "development_method": {
                ...
                "statement": (
                    "Sistema disenado, dirigido, curado y refinado por Xyphrnyx "
            ...
            "attribution_limits": {
                "statement": (
                    ...
                    "a Xyphrnyx el material fuente de usuarios o terceros."
                )
            },
        },
        "engine": {
...
def build_manifest_document(
    ...
    *,
    ...
    proof = {
        ...
        "signature": signature,
        "signed_scope": (
            "CANONICAL_XYPHERNYX_PROVENANCE_CORE"
            if signature
            else None
        ),
    }
...
def compact_hash(value: str, width: int = 18) -> str:
    # Se conserva el nombre y la firma por compatibilidad,
    # pero se devuelve el hash completo (sin truncar).
    return value
...
def signature_page_bytes(
    ...
    *,
    ...
    document = SimpleDocTemplate(
        ...
        topMargin=18 * mm,
        bottomMargin=18 * mm,
        title="Autoria, procedencia e integridad Xyphrnyx",
    )

    styles = getSampleStyleSheet()
    title_style = ParagraphStyle("XyphrnyxTitle", parent=styles["Heading1"], fontName="Helvetica-Bold", fontSize=16, leading=19, textColor=colors.HexColor("#1f2937"), spaceAfter=10)
    subtitle_style = ParagraphStyle("XyphrnyxSubtitle", parent=styles["Normal"], fontName="Helvetica", fontSize=9, leading=13, textColor=colors.HexColor("#4b5563"), spaceAfter=12)
    body_style = ParagraphStyle("XyphrnyxBody", parent=styles["Normal"], fontName="Helvetica", fontSize=8.5, leading=12, alignment=TA_LEFT, textColor=colors.HexColor("#374151"))
    small_style = ParagraphStyle("XyphrnyxSmall", parent=body_style, fontSize=7.7, leading=10.5, textColor=colors.HexColor("#6b7280"))

    story: list[Any] = [
        Paragraph("AUTORIA, PROCEDENCIA E INTEGRIDAD", title_style),
        Paragraph("Manifiesto visible incorporado por el posprocesador local Xyphrnyx. Las paginas fuente se conservan y esta pagina documenta la intervencion posterior.", subtitle_style),
    ]
    ...
    story.append(Paragraph("<b>Arquitectura, direccion intelectual, curaduria y evolucion iterativa: Xyphrnyx.</b>", body_style))
    ...
    story.append(Spacer(1, 3 * mm))
    story.append(Paragraph("Esta marca acredita la arquitectura, metodologia y procedencia tecnica del procesamiento. No atribuye automaticamente a Xyphrnyx la autoria del material fuente proporcionado por usuarios o terceros.", small_style))
...
def append_signature_page(source_path: Path, output_path: Path, page_pdf_bytes: bytes) -> None:
    try:
        ...
        raise XyphrnyxError(f"No fue posible leer el PDF fuente: {source_path}") from exc

    if source_reader.is_encrypted:
        try:
            result = source_reader.decrypt("")
        except Exception as exc:
            raise XyphrnyxError("El PDF esta cifrado y no puede procesarse sin clave.") from exc
        if result == 0:
            raise XyphrnyxError("El PDF esta cifrado y requiere contrasena.")
    ...
    if source_reader.metadata:
        ...
        metadata["/XyphrnyxPostprocessor"] = f"{APP_NAME} {APP_VERSION}"
        writer.add_metadata(metadata)
    else:
        writer.add_metadata({"/XyphrnyxPostprocessor": f"{APP_NAME} {APP_VERSION}"})
...
def verify_package(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not provenance_path.exists():
        raise XyphrnyxError("No se encontro el archivo de procedencia.")
...
def main() -> int:
    ...
    try:
        ...
        return 0
    except (XyphrnyxError, OSError, json.JSONDecodeError) as exc:
